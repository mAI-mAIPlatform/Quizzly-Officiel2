    ...
    <div className={`glass p-6 flex flex-col gap-4 relative overflow-hidden transition-all duration-300 ${quest.isCompleted ? "opacity-50 grayscale" : ""}`}>
      ...
      <div className="space-y-1">
        ...
        <div className="w-full h-2 bg-foreground/10 rounded-full overflow-hidden">
          <div className="h-full transition-all duration-500 bg-primary" style={{ width: `${progressPercent}%` }} />
        </div>
      </div>
      ...
      <button
        onClick={() => {
          ...
          if (quest.id === "q_weekly_1" && isReady) {
...
export default function QuetesPage() {
  const { progress, claimQuestReward, claimChest } = useProgress();

  const dailyQuests = progress.quests.filter((quest) => quest.id.startsWith("q_daily"));
  const weeklyQuests = progress.quests.filter((quest) => quest.id.startsWith("q_weekly"));
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20">
        ...
        <section className="space-y-6">
          ...
          <div className="flex flex-col gap-4">
            {dailyQuests.map((quest) => (
              <QuestCard key={quest.id} quest={quest} claimQuestReward={claimQuestReward} claimChest={claimChest} />
            ))}
          </div>
        </section>
      ...
      <div className="glass p-10 bg-gradient-to-br from-primary/10 to-transparent text-center border-primary/20 rounded-[2rem] shadow-xl relative overflow-hidden">
        ...
        <p className="opacity-60 mb-6 font-medium relative z-10">Les quêtes journalières et hebdomadaires se réinitialisent automatiquement pour te garder en mouvement !</p>
